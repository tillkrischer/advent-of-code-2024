...
const reachableDests: number[][] = new Array(nums.length)
  .fill(0)
  .map((_) => new Array(nums[0].length).fill(0));
...
function getReachable(y: number, x: number, value: number): number {
  const directions = [
    [0, -1],
    [0, 1],
    [-1, 0],
  ...
  for (let [dy, dx] of directions) {
    ...
    if (
      ...
      reachable += reachableDests[ny][nx];
    }
  }
