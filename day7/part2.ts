...
function test(result: number, operands: number[]): boolean {
  ...
  const conc = Number.parseInt(operands[0].toString() + operands[1].toString());
  const concatOps = [conc, ...operands.slice(2)];

  return (
    test(result, addOps) || test(result, mulOps) || test(result, concatOps)
  );
}
